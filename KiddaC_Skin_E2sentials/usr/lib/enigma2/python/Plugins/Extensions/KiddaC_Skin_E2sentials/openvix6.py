...
from enigma import iServiceInformation, RT_HALIGN_LEFT, RT_HALIGN_RIGHT, RT_VALIGN_CENTER, BT_SCALE, BT_KEEP_ASPECT_RATIO, BT_HALIGN_CENTER, BT_ALIGN_CENTER, BT_VALIGN_CENTER, eServiceReference, eServiceCenter
...
try:
    from Components.MovieList import MovieListData, StubInfo, getItemDisplayName, moviePlayState
except:
    try:
        from Components.MovieList import MovieListData, StubInfo, moviePlayState
    except:
        pass
try:
...
def openvix6BuildMovieListEntry(self, serviceref, info, begin, data):
    # #################### mod ###################### #
    ...
    if data.dirty:
        ...
        if os.path.split(pathName)[1].rsplit(".", 1)[0] in self.runningTimers:
            if switch == 'i':
